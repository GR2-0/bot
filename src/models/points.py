...
class PointsManager:
    ...
    def get_user_points(self, user_id: str) -> int:
        ...
        print(f"DEBUG: PointsManager.get_user_points({user_id}) = {points}")
        print(f"DEBUG: All points data: {self.points}")
        return points

    def add_points(self, user_id: str, amount: int, action: str = "unknown"):
    ...
    def remove_points(self, user_id: str, amount: int, action: str = "unknown"):
    ...
    def transfer_points(self, from_user_id: str, to_user_id: str, amount: int, reason: str = "Перевод"):
        ...
        # Выполняем перевод
        self.remove_points(from_user_id, amount, "transfer_out")
        self.add_points(to_user_id, amount, "transfer_in")
    ...
    def reset_user_points(self, user_id: str, reason: str = "Сброс поинтов"):
        ...
        current_points = self.get_user_points(user_id)
        if current_points > 0:
            self.remove_points(user_id, current_points, "reset")
    ...
    def award_points_for_action(self, user_id: str, action: str):
        ...
        if points > 0:
            self.add_points(user_id, points, action)
